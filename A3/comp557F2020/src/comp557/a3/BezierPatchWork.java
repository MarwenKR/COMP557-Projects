...
public class BezierPatchWork {
	...
	private Vector3d evaluateCoordinate( double s, double t, int patch ) {
    	...
    	double x = 0, y = 0, z = 0;
    	double[] B_s =  {(1-s)*(1-s)*(1-s),3*s*(1-s)*(1-s),3*s*s*(1-s),s*s*s};
    	double[] B_t = {(1-t)*(1-t)*(1-t),3*t*(1-t)*(1-t),3*t*t*(1-t),t*t*t};
    		
    	for(int i = 0; i<4; i++) {	   
    		for(int j = 0; j<4; j++) {	    
		    	 x += B_s[i]*B_t[j]*coordinatePatch[patch][0].getElement(i, j);
		    	 y += B_s[i]*B_t[j]*coordinatePatch[patch][1].getElement(i, j);
		    	 z += B_s[i]*B_t[j]*coordinatePatch[patch][2].getElement(i, j);
    		}
    	}
	...
	private Vector3d differentiateS(double s,double t,int patch) {
    	...
    	double x = 0, y = 0, z = 0;
    	double[] dB_s =   {-3*(1-s)*(1-s),3*(1-3*s)*(1-s),3*s*(2-3*s),3*s*s};
    	double[] B_t = {(1-t)*(1-t)*(1-t),3*t*(1-t)*(1-t),3*t*t*(1-t),t*t*t};
    		
    	for(int i = 0; i<4; i++) {	   
    		for(int j = 0; j<4; j++) {	    
		    	 x += dB_s[i]*B_t[j]*coordinatePatch[patch][0].getElement(i, j);
		    	 y += dB_s[i]*B_t[j]*coordinatePatch[patch][1].getElement(i, j);
		    	 z += dB_s[i]*B_t[j]*coordinatePatch[patch][2].getElement(i, j);
    		}
    	}
	...
	private Vector3d differentiateT(double s,double t,int patch) {
    	...
    	double x = 0, y = 0, z = 0;
    	double[] B_s =  {(1-s)*(1-s)*(1-s),3*s*(1-s)*(1-s),3*s*s*(1-s),s*s*s};
    	double[] dB_t = {-3*(1-t)*(1-t),3*(1-3*t)*(1-t),3*t*(2-3*t),3*t*t};
    		
    	for(int i = 0; i<4; i++) {	   
    		for(int j = 0; j<4; j++) {	    
		    	 x += B_s[i]*dB_t[j]*coordinatePatch[patch][0].getElement(i, j);
		    	 y += B_s[i]*dB_t[j]*coordinatePatch[patch][1].getElement(i, j);
		    	 z += B_s[i]*dB_t[j]*coordinatePatch[patch][2].getElement(i, j);
    		}
    	}
		return new Vector3d(x,y,z);
	}
	...
	private Vector3d evalNormal(double s, double t, int patch) {
		...
		Vector3d u = differentiateS( s, t, patch);
		Vector3d v = differentiateT( s, t, patch); 
		Vector3d w = new Vector3d();
		w.cross(u, v);
				
	
		if(w.length()< 1e-9) {
			if (Math.abs(t-0.5)<1e-9 && v.length()<1e-9 ){
			 t = t+1e-4;
			 v = differentiateT( s, t, patch);
			 }
			
			if (Math.abs(s-0.5)<1e-9 && u.length()<1e-9 ){
				 s = s+1e-4;
				 u = differentiateS( s, t, patch);
				 }
			w.cross(u, v);
