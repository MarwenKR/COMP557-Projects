...
public class Parser {
	...
	public static Light createLight(Node dataNode) {
        ...
        if ( samplesAttr != null ) {
            light.samples = Integer.parseInt( samplesAttr.getNodeValue() );        	
        }
        
        Node uAttr = dataNode.getAttributes().getNamedItem("u");
        if ( uAttr != null ) {
        	Scanner s = new Scanner( uAttr.getNodeValue());
            double x = s.nextDouble();
            double y = s.nextDouble();
            double z = s.nextDouble();
            light.u.set(x,y,z); 
        ...
        Node vAttr = dataNode.getAttributes().getNamedItem("v");
        if ( vAttr != null ) {
        	Scanner s = new Scanner( vAttr.getNodeValue());
            double x = s.nextDouble();
            double y = s.nextDouble();
            double z = s.nextDouble();
            light.v.set(x,y,z); 
	...
	public static Camera createCamera(Node dataNode) {
        ...
        Node focusAttr = dataNode.getAttributes().getNamedItem("focus");
        if ( focusAttr != null ) {
            camera.focus = Double.parseDouble( focusAttr.getNodeValue() );       	
        }
        Node BlurAttr = dataNode.getAttributes().getNamedItem("blur");
        if ( BlurAttr != null ) {
            camera.Blur = Double.parseDouble( BlurAttr.getNodeValue() );       	
        }
        Node samplesAttr = dataNode.getAttributes().getNamedItem("eyeSamples");
		if ( samplesAttr != null ) {
        	Scanner s = new Scanner( samplesAttr.getNodeValue());
            camera.eyeSamples = s.nextInt(); 
			s.close();
		}
	...
	public static Material createMaterial(Node dataNode) {
		...
		if( refAttr  != null ) {
	    	...
	    	Node refractiveAttr = dataNode.getAttributes().getNamedItem("refractive");
	    	if ( refractiveAttr != null ) {
	        	Scanner s = new Scanner( refractiveAttr.getNodeValue());
	            float r = s.nextFloat();
	            float g = s.nextFloat();
	            float b = s.nextFloat();
	            float a = (s.hasNextFloat() ? s.nextFloat() : 1);
	            material.refractive.set(r,g,b,a);   
	            s.close();
	    	};
	    	
	    	Node nAttr = dataNode.getAttributes().getNamedItem("n");
	    	if ( nAttr != null ) {
	    		material.n = Float.parseFloat( nAttr.getNodeValue() );
	    	}
